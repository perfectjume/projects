...
m=json.loads(z.read('hit_indicator.mixins.json'))
assert 'client.EmfModelAnimationFreezeMixin' in m.get('client', []), m
for p in [
...
echo "EMF_POSE_PARTSTATE_SELFTEST_PASS" >> runtime-summary.txt
echo "OPTIONAL_EMF_POSE_MIXIN_ABSENT_PASS" >> runtime-summary.txt
echo "IMMEDIATE_FREEZE_NO_2TICK_DELAY_PASS" >> runtime-summary.txt

python3 - <<'PY'
from pathlib import Path
import zipfile
out=Path('/tmp/hit-indicator-v25-emf-pose-freeze-evidence.zip')
paths=[
