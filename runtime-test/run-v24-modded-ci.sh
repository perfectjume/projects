...
echo "=== V24 EMF MODDED reconstruct ==="
(
set -euo pipefail
cat \
...
echo "=== V24 EMF MODDED prepare/build ==="
(
set -euo pipefail
...
cp ../runtime-test/EmfStasisCompat.java src/main/java/com/misanthropy/hit_indicator/client/EmfStasisCompat.java
...
test -f build/classes/java/main/com/misanthropy/hit_indicator/client/EmfStasisCompat.class
...
python ../runtime-test/PackageV24.py \
  /tmp/hit_indicator-1.21.1-neoforge-v23.jar \
  build/classes/java/main/com/misanthropy/hit_indicator/client/EmfStasisCompat.class \
  build/classes/java/main/com/misanthropy/hit_indicator/client/StasisDesaturationClientEvents.class \
...
javap -classpath /tmp/hit_indicator-1.21.1-neoforge-v24.jar -c -p \
  com.misanthropy.hit_indicator.client.EmfStasisCompat > /tmp/emf-stasis-v24.javap
grep -q 'traben.entity_model_features.EMFAnimationApi' /tmp/emf-stasis-v24.javap
grep -q 'registerPauseCondition' /tmp/emf-stasis-v24.javap
grep -q 'WindupTracker.shouldFreeze' /tmp/emf-stasis-v24.javap
grep -q 'FrozenRenderClock.isReleaseBridge' /tmp/emf-stasis-v24.javap
python - <<'PY'
import zipfile
z=zipfile.ZipFile('/tmp/hit_indicator-1.21.1-neoforge-v24.jar')
for p in [
 'com/misanthropy/hit_indicator/client/EmfStasisCompat.class',
 'com/misanthropy/hit_indicator/client/StasisDesaturationClientEvents.class',
 'META-INF/hit-indicator-v24-marker.txt',
]:
    assert p in z.namelist(), p
print('V24_EMF_STASIS_PACKAGE_PASS')
PY
...
rm -f src/main/java/com/misanthropy/hit_indicator/client/EmfStasisCompat.java
rm -f build/classes/java/main/com/misanthropy/hit_indicator/client/EmfStasisCompat.class
...
# compileOnly/runtime now point at the final v24 EMF-stasis-freeze binary.
# The predictor source/classes are compiler-only intermediates and are removed below.
cp /tmp/hit_indicator-1.21.1-neoforge-v24.jar libs/
sed -i 's/hit_indicator-1.21.1-neoforge-v5.jar/hit_indicator-1.21.1-neoforge-v24.jar/g' build.gradle

rm -f run/mods/hit_indicator-1.21.1-neoforge-v5.jar
cp /tmp/hit_indicator-1.21.1-neoforge-v24.jar run/mods/
)

echo "=== V24 EMF MODDED world generation ==="
(
set -uo pipefail
cd mdk
mkdir -p run
...
echo "=== V24 EMF MODDED assertions ==="
(
set -euo pipefail
...
grep -q '\[HI-MATRIX\] EMF_STASIS_COMPAT registered=true' client-latest.log
grep -q '\[HI-MATRIX\] EMF_STASIS_FROZEN_CONDITION paused=true' client-latest.log
grep -q '\[HI-MATRIX\] EMF_STASIS_RESUME_CONDITION paused=false' client-latest.log
...
echo "EXACT_EMF_ETF_STASIS_COMPAT_PASS" >> runtime-summary.txt
echo "IMMEDIATE_FREEZE_NO_2TICK_DELAY_PASS" >> runtime-summary.txt

python3 - <<'PY'
from pathlib import Path
import zipfile
out=Path('/tmp/hit-indicator-v24-emf-stasis-freeze-modded-evidence.zip')
paths=[
    Path('/tmp/hit_indicator-1.21.1-neoforge-v24.jar'),
