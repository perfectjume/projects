...
cp /tmp/hit_indicator-1.21.1-neoforge-v23.jar libs/
sed -i 's/hit_indicator-1.21.1-neoforge-v5.jar/hit_indicator-1.21.1-neoforge-v23.jar/g' build.gradle

rm -f run/mods/hit_indicator-1.21.1-neoforge-v5.jar
cp /tmp/hit_indicator-1.21.1-neoforge-v23.jar run/mods/
)

echo "=== V23 world generation ==="
(
set -uo pipefail
cd mdk
mkdir -p run
...
grep -Eq '\[HI-MATRIX\] STASIS_GRAYSCALE shaderReady=true stencilReady=true beginCount=[1-9][0-9]* passCount=[1-9][0-9]* executed=true' client-latest.log
...
import zipfile
out=Path('/tmp/hit-indicator-v23-universal-stasis-grayscale-evidence.zip')
paths=[
    Path('/tmp/hit_indicator-1.21.1-neoforge-v23.jar'),
