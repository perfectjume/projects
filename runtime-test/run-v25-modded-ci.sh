...
echo "=== V25 EMF CLOCK MODDED reconstruct ==="
(
set -euo pipefail
cat \
...
echo "=== V25 EMF CLOCK MODDED prepare/build ==="
(
set -euo pipefail
...
cp ../runtime-test/EntityRenderContext.java src/main/java/com/misanthropy/hit_indicator/client/EntityRenderContext.java
cp ../runtime-test/EmfAnimationClockCompat.java src/main/java/com/misanthropy/hit_indicator/client/EmfAnimationClockCompat.java
cp ../runtime-test/EmfAnimationClockMixin.java src/main/java/com/misanthropy/hit_indicator/mixin/client/EmfAnimationClockMixin.java
...
test -f build/classes/java/main/com/misanthropy/hit_indicator/client/EntityRenderContext.class
test -f build/classes/java/main/com/misanthropy/hit_indicator/client/EmfAnimationClockCompat.class
test -f build/classes/java/main/com/misanthropy/hit_indicator/mixin/client/EmfAnimationClockMixin.class
...
python ../runtime-test/PackageV25.py \
  /tmp/hit_indicator-1.21.1-neoforge-v23.jar \
  build/classes/java/main/com/misanthropy/hit_indicator/client/EntityRenderContext.class \
  build/classes/java/main/com/misanthropy/hit_indicator/client/EmfAnimationClockCompat.class \
  build/classes/java/main/com/misanthropy/hit_indicator/mixin/client/EmfAnimationClockMixin.class \
  build/classes/java/main/com/misanthropy/hit_indicator/mixin/client/LevelRendererTimeStopMixin.class \
...
javap -classpath /tmp/hit_indicator-1.21.1-neoforge-v25.jar -c -p \
  com.misanthropy.hit_indicator.client.EmfAnimationClockCompat > /tmp/emf-clock-compat-v25.javap
javap -classpath /tmp/hit_indicator-1.21.1-neoforge-v25.jar -v -p \
  com.misanthropy.hit_indicator.mixin.client.EmfAnimationClockMixin > /tmp/emf-clock-mixin-v25.javap
grep -q 'adjustTickDelta' /tmp/emf-clock-mixin-v25.javap
grep -q 'adjustFrameTime' /tmp/emf-clock-mixin-v25.javap
grep -q 'adjustTime' /tmp/emf-clock-mixin-v25.javap
grep -q 'adjustFrameCounter' /tmp/emf-clock-mixin-v25.javap
python - <<'PY'
import json,zipfile
z=zipfile.ZipFile('/tmp/hit_indicator-1.21.1-neoforge-v25.jar')
m=json.loads(z.read('hit_indicator.mixins.json'))
assert 'client.EmfAnimationClockMixin' in m.get('client', []), m
for p in [
 'com/misanthropy/hit_indicator/client/EntityRenderContext.class',
 'com/misanthropy/hit_indicator/client/EmfAnimationClockCompat.class',
 'com/misanthropy/hit_indicator/mixin/client/EmfAnimationClockMixin.class',
 'META-INF/hit-indicator-v25-marker.txt',
]:
    assert p in z.namelist(), p
assert 'com/misanthropy/hit_indicator/client/EmfStasisCompat.class' not in z.namelist()
print('V25_EMF_CLOCK_FREEZE_PACKAGE_PASS')
PY
...
rm -f src/main/java/com/misanthropy/hit_indicator/client/EntityRenderContext.java
rm -f build/classes/java/main/com/misanthropy/hit_indicator/client/EntityRenderContext.class
rm -f src/main/java/com/misanthropy/hit_indicator/client/EmfAnimationClockCompat.java
rm -f build/classes/java/main/com/misanthropy/hit_indicator/client/EmfAnimationClockCompat.class
rm -f src/main/java/com/misanthropy/hit_indicator/mixin/client/EmfAnimationClockMixin.java
rm -f build/classes/java/main/com/misanthropy/hit_indicator/mixin/client/EmfAnimationClockMixin.class
...
# compileOnly/runtime now point at the final v25 EMF-clock-freeze binary.
...
echo "=== V25 EMF CLOCK MODDED world generation ==="
(
set -uo pipefail
cd mdk
mkdir -p run
...
echo "=== V25 EMF/ETF client-only dependency setup ==="
(
set -euo pipefail
cd mdk
cat >> build.gradle <<'GRADLE'
...
echo "=== V25 EMF CLOCK MODDED literal client ==="
(
set -uo pipefail
cd mdk
mkdir -p ../screenshots
...
echo "=== V25 EMF CLOCK MODDED assertions ==="
(
set -euo pipefail
...
echo "EXACT_EMF_ETF_CLOCK_FREEZE_PASS" >> runtime-summary.txt
echo "IMMEDIATE_FREEZE_NO_2TICK_DELAY_PASS" >> runtime-summary.txt

python3 - <<'PY'
from pathlib import Path
import zipfile
out=Path('/tmp/hit-indicator-v25-emf-clock-freeze-modded-evidence.zip')
paths=[
    ...
    Path('/tmp/emf-clock-compat-v25.javap'),
    Path('/tmp/emf-clock-mixin-v25.javap'),
