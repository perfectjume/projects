...
# compileOnly/runtime now point at the final v22 continuous-resume binary.
# The predictor source/classes are compiler-only intermediates and are removed below.
cp /tmp/hit_indicator-1.21.1-neoforge-v22.jar libs/
sed -i 's/hit_indicator-1.21.1-neoforge-v5.jar/hit_indicator-1.21.1-neoforge-v22.jar/g' build.gradle

rm -f run/mods/hit_indicator-1.21.1-neoforge-v5.jar
cp /tmp/hit_indicator-1.21.1-neoforge-v22.jar run/mods/
)

echo "=== V22 world generation ==="
(
set -uo pipefail
cd mdk
mkdir -p run
...
import zipfile
out=Path('/tmp/hit-indicator-v22-continuous-resume-evidence.zip')
paths=[
    Path('/tmp/hit_indicator-1.21.1-neoforge-v22.jar'),
...
from pathlib import Path
import zipfile
out=Path('/tmp/hit-indicator-v21-stable-time-stop-evidence.zip')
paths=[
    Path('/tmp/hit_indicator-1.21.1-neoforge-v21.jar'),
    Path('/tmp/windup-v20.javap'),
    Path('/tmp/clientlevel-time-stop-v21.javap'),
    Path('/tmp/levelrenderer-time-stop-v21.javap'),
    Path('/tmp/frozen-maintenance-v21.javap'),
