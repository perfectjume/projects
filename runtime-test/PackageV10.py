...
if len(sys.argv) != 4:
    raise SystemExit('usage: PackageV10.py <v6.jar> <patched-renderer.class> <out.jar>')
...
transparent_icon = png_bytes(Image.new('RGBA', (16,16), (0,0,0,0)))

icon_paths = {
    'assets/hit_indicator/textures/indicator/icon.png',
...
with zipfile.ZipFile(base_jar, 'r') as zin, zipfile.ZipFile(out_jar, 'w', compression=zipfile.ZIP_DEFLATED) as zout:
    seen=set()
    for info in zin.infolist():
        if info.filename in seen or info.filename in replace:
            continue
    ...
    for path in sorted(icon_paths):
        zout.writestr(path, transparent_icon)
