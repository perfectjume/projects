...
# compileOnly/runtime now point at the final v21 stable time-stop binary.
# The predictor source/classes are compiler-only intermediates and are removed below.
cp /tmp/hit_indicator-1.21.1-neoforge-v21.jar libs/
sed -i 's/hit_indicator-1.21.1-neoforge-v5.jar/hit_indicator-1.21.1-neoforge-v21.jar/g' build.gradle

rm -f run/mods/hit_indicator-1.21.1-neoforge-v5.jar
cp /tmp/hit_indicator-1.21.1-neoforge-v21.jar run/mods/
)

echo "=== V21 world generation ==="
(
set -uo pipefail
cd mdk
mkdir -p run
