...
grep -q 'entity_model_features' /tmp/emf-stasis-v24.javap
grep -q 'registerPauseCondition' /tmp/emf-stasis-v24.javap
python - <<'PY'
import zipfile
z=zipfile.ZipFile('/tmp/hit_indicator-1.21.1-neoforge-v24.jar')
assert 'com/misanthropy/hit_indicator/client/EmfStasisCompat.class' in z.namelist()
assert 'META-INF/hit-indicator-v24-marker.txt' in z.namelist()
print('V24_OPTIONAL_EMF_PACKAGE_PASS')
PY
...
echo "OPTIONAL_EMF_COMPAT_ABSENT_PASS" >> runtime-summary.txt
echo "IMMEDIATE_FREEZE_NO_2TICK_DELAY_PASS" >> runtime-summary.txt

python3 - <<'PY'
from pathlib import Path
import zipfile
out=Path('/tmp/hit-indicator-v24-emf-stasis-freeze-evidence.zip')
paths=[
