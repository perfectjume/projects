import base64, io, math, sys, zipfile
from pathlib import Path
from PIL import Image

if len(sys.argv) != 4:
    raise SystemExit('usage: v8_runtime_patch.py <v6.jar> <renderer.class.b64> <out.jar>')
base_jar = Path(sys.argv[1])
renderer_b64 = Path(sys.argv[2])
...
def ring(radius, fixed=False):
    ...
    for y in range(N):
        for x in range(N):
            dx=x-C; dy=y-C; r=math.hypot(dx,dy); a=(math.degrees(math.atan2(dy,dx))+360)%360
            color=None
            if fixed:
            ...
            if color: im.putpixel((x,y),color)
    return im
...
renderer = base64.b64decode(renderer_b64.read_text().strip())
